import { Injectable } from '@angular/core';

@Injectable({
  providedIn: 'root'
})
export class JogodavelhaService {

  private readonly TAM_TAB: number = 3;
  private readonly X: number = 1;
  private readonly O: number = 2;
  private readonly VAZIO: number = 0;

  private tabuleiro: any;
  private numMovimentos: number;
  private vitoria: any;

  private _jogador: number;
  private _showInicio: boolean;
  private _showTabuleiro: boolean;
  private _showFinal: boolean;

  constructor() { }

  inicializador(): void {
    this._showInicio = true;
    this._showTabuleiro = false;
    this._showFinal = false;
    this.vitoria = false;
    this._jogador = this.X
    this.numMovimentos = 0;
    this.inicializarTab();
  }

  inicializarTab(): void {
    this.tabuleiro = [this.TAM_TAB];
    for (let i = 0; i < this.TAM_TAB; i++) {
      this.tabuleiro[i] = [this.VAZIO, this.VAZIO, this.VAZIO];
    }
  }

  get showInicio(): boolean {
    return this._showInicio;
  }

  get showTabuleiro(): boolean {
    return this._showTabuleiro;
  }

  get showFinal(): boolean {
    return this._showFinal;
  }

  get jogador(): number {
    return this._jogador;
  }

  inicioJogo(): void {
    this._showInicio = false;
    this._showTabuleiro = true;
  }

  jogar(posX: number, posY: number): void {
    //jogada invalida
    if (this.tabuleiro[posX][posY] !== this.VAZIO || this.vitoria) {
      return;
    }

    this.tabuleiro[posX][posY] = this._jogador;

    //inicia os movimentos no tabuleiro
    this.numMovimentos++ //this.numMovimentos = this.numMovimentos + 1;

    //verifica se houve vitoria apos os moviemntos e finaliza o jogo de caso positivo.
    this.vitoria = this.fimJogo(posX, posY, this.tabuleiro, this._jogador);

    //troca de jogador
    this._jogador = (this._jogador === this.X) ? this.O : this.X;

    //verifica se jogo terminou 
    if (!this.vitoria && this.numMovimentos < 9) {
      this.cpuJogar();
    }

    //verifica se houve vitoria 
    if (this.vitoria !== false) {
      this._showFinal = true;
    }

    //verifica empate  
    if (!this.vitoria && this.numMovimentos === 9) {
      this._jogador = 0;
      this._showFinal = true;
    }
  }

  fimJogo(linha: number, coluna: number,
    tabuleiro: any, jogador: number): void {
    let fim: any = false;

    //validar as linhas
    if (tabuleiro[linha][0] === jogador &&
      tabuleiro[linha][1] === jogador &&
      tabuleiro[linha][2] === jogador) {
      fim = [[linha, 0], [linha, 1], [linha, 2]]
    }
    //validar as coluna
    if (tabuleiro[coluna][0] === jogador &&
      tabuleiro[coluna][1] === jogador &&
      tabuleiro[coluna][2] === jogador) {
      fim = [[coluna, 0], [coluna, 1], [coluna, 2]]
    }
    //validar diagonais
    //#region validarDiagonais
    //Zero
    if (tabuleiro[0][0] === jogador &&
      tabuleiro[0][1] === jogador &&
      tabuleiro[0][2] === jogador) {
      fim = [[0, 0], [0, 1], [0, 2]]
    }

    if (tabuleiro[0][0] === jogador &&
      tabuleiro[1][0] === jogador &&
      tabuleiro[2][0] === jogador) {
      fim = [[0, 0], [1, 0], [2, 0]]
    }

    if (tabuleiro[0][0] === jogador &&
      tabuleiro[1][1] === jogador &&
      tabuleiro[2][2] === jogador) {
      fim = [[0, 0], [1, 1], [2, 2]]
    }

    //um
    if (tabuleiro[1][0] === jogador &&
      tabuleiro[1][1] === jogador &&
      tabuleiro[1][2] === jogador) {
      fim = [[1, 0], [1, 1], [1, 2]]
    }

    if (tabuleiro[2][0] === jogador &&
      tabuleiro[2][1] === jogador &&
      tabuleiro[2][2] === jogador) {
      fim = [[2, 0], [2, 1], [2, 2]]
    }

    if (tabuleiro[2][0] === jogador &&
      tabuleiro[1][1] === jogador &&
      tabuleiro[0][2] === jogador) {
      fim = [[2, 0], [1, 1], [0, 2]]
    }

    if (tabuleiro[0][1] === jogador &&
      tabuleiro[1][1] === jogador &&
      tabuleiro[2][1] === jogador) {
      fim = [[0, 1], [1, 1], [2, 1]]
    }

    if (tabuleiro[0][2] === jogador &&
      tabuleiro[1][2] === jogador &&
      tabuleiro[2][2] === jogador) {
      fim = [[0, 2], [1, 2], [2, 2]]
    }
    //#endregion

    return fim;    
  }

  cpuJogar(): void{
    //verifica jogada de vitoria
    let jogada: number[] = this.obterJogada(this.O);
    if(jogada.length <= 0){
      jogada = this.obterJogada(this.X);      
    }
    if(jogada.length <= 0){
      //joga aleatoria
      let jogadas: any =[];
      for (let index = 0; index <this.TAM_TAB; index++) {
        for (let indexJ = 0; indexJ <this.TAM_TAB; indexJ++) {       
          if(this.tabuleiro[index][indexJ] === this.VAZIO){
            jogadas.push([index, indexJ])
          }
        }
      }
      let k = Math.floor((Math.random() * (jogadas.length - 1)));
      jogada = [jogadas[k][0], jogadas[k][1]];
    }

    this.tabuleiro[jogada[0]][jogada[1]] = this._jogador;

    this.numMovimentos++ //this.numMovimentos = this.numMovimentos + 1;

    //verifica se houve vitoria apos os moviemntos e finaliza o jogo de caso positivo.
    this.vitoria = this.fimJogo(jogada[0], jogada[1], this.tabuleiro, this._jogador);

    //troca de jogador
    this._jogador = (this._jogador === this.X) ? this.O : this.X;
  }

  obterJogada(jogador: number): number[]{
    let tab = this.tabuleiro;
    let fim: any = false;

    for (let lin = 0; lin <this.TAM_TAB; lin++) {
      for (let col = 0; col <this.TAM_TAB; col++) {       
        if(tab[lin][col] !== this.VAZIO){
         continue;
        }
        tab[lin][col] = jogador;
        fim = this.fimJogo(lin, col, tab, jogador)
        if(fim){
          return [lin, col];
        }
        tab[lin][col] = this.VAZIO;
      }
    }
    return [];
  }

  exibirX(posX:number, posY:number): boolean{
    return this.tabuleiro[posX][posY] === this.X;
  }

  exibirO(posX:number, posY:number): boolean{
    return this.tabuleiro[posX][posY] === this.O;
  }

  exibirVitoria(posX: number, posY:number): boolean{
    let exibirVitoria: boolean = false;

    if(!this.vitoria){
      return exibirVitoria;
    }
    for(let pos of this.vitoria){
      if(pos[0] === posX && pos[1] === posY){
        exibirVitoria = true;
        break;
      }
    }
    return exibirVitoria;
  }

  novoJogo(): void{
    this.inicializador();
    //this._showFinal = false;
    this._showInicio = false;
    this._showTabuleiro = true;
  }
  
}
